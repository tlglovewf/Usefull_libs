...
namespace MyInterface {
    ...
    DynamicLibrary::DynamicLibrary(const std::string& name, HANDLE handle)
    {
        _name = name;
        _handle = handle;
        std::cout << "Opened DynamicLibrary " << _name << std::endl;
    }
    ...
    DynamicLibrary::~DynamicLibrary()
    {
        if (_handle)
        {
            std::cout << "Closing DynamicLibrary " << _name << std::endl;
#if defined(WIN32) && !defined(__CYGWIN__)
    ...
    DynamicLibrary* DynamicLibrary::loadLibrary(const std::string& libraryName)
    {
        ...
        std::cout << "DynamicLibrary::try to load library \"" << libraryName << "\"" << std::endl;

       // std::string fullLibraryName = findLibraryFile(libraryName);
       ...
       //else handle = getLibraryHandle(libraryName); // haven't found a lib ourselves, see if the OS can find it simply from the library name.
        ...
        std::cout << "DynamicLibrary::failed loading \"" << libraryName << "\"" << std::endl;
    ...
    static inline T convertPointerType(R src)
    {
        T dest;
